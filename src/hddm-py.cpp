 ...
 */
   ...
   pysetup <<
   "#\n"
   "# This file was generated by the hddm-py utility whose source\n"
   "# lives in src/programs/Utilities/hddm/hddm-py.cpp\n"
   "#\n"
   "# If you value your sanity, do NOT attempt to understand how \n"
   "# this Rube-Goldberg build system works!\n"
   "#\n"
   "import os\n"
   ...
   "my_include_dirs = [os.environ['HALLD_RECON_HOME'] + '/' +\n"
   "                      os.environ['BMS_OSNAME'] + '/include',\n"
   "                   os.environ['HALLD_RECON_HOME'] + \n"
   "                      '/src/libraries/include',\n"
   "                   source_dir,\n"
   "                  ]\n"
   "my_library_dirs = [os.environ['HALLD_RECON_HOME'] + '/' +\n"
   "                      os.environ['BMS_OSNAME'] + '/lib',\n"
   "                  ]\n"
   "my_libraries = ['HDDM', 'xstream', 'bz2', 'z']\n"
   "my_extra_cxxflags = ['-std=c++11']\n"
   "if 'HDF5ROOT' in os.environ:\n"
   "   hdf5_root = os.environ['HDF5ROOT']\n"
   "   my_include_dirs += [hdf5_root + '/include']\n"
   "   my_library_dirs += [hdf5_root + '/lib']\n"
   "   my_libraries += ['hdf5_cpp', 'hdf5_hl', 'hdf5', 'sz', 'dl']\n"
   ...
   "       ext_modules = [module1])\n"
   "	print('setup suceeded!')\n"
   "\n"
   "except BaseException:\n"
   "    # n.b. The following only seems to print when scons was run with SHOWBUILD=1\n"
   "	print('')\n"
   "	print('ERROR occurred while attempting to build python module.')\n"
   "	print('      This may be due to any number of reasons. It may')\n"
   "	print('      be related to the compiler since this requires gcc')\n"
   "	print('      so we force the usage of g++ from your PATH which')\n"
   "	print('      may be different than what you specified via environment.')\n"
   "	print('      To fix the error, you should look at the file:')\n"
   "	print('          src/programs/Utilities/hddm/hddm-py.cpp')\n"
   "	print('')\n"
   "\n"
